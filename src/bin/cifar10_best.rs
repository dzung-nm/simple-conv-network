...
/// Currently it can archive 71.18% accuracy with num_samples_per_epoch = 20.000
/// Epoch 030: time = 13.335s, Validation Accuracy: 71.18%
...
fn main() {
    ...
    let layers: Vec<Box<dyn Layer>> = vec![
        Box::new(ConvLayer::new(&cv_layer1)), // → 12×32×32
        Box::new(MaxPoolLayer::new(&pl_layer1)), // → 12×16×16
        Box::new(ConvLayer::new(&cv_layer2)), // → 16×16×16
        Box::new(MaxPoolLayer::new(&pl_layer2)), // → 16×8×8
        Box::new(ConvLayer::new(&cv_layer3)), // → 16×8×8
        Box::new(MaxPoolLayer::new(&pl_layer3)), // → 16×4×4
        Box::new(FullyConnectedLayer::with_dropout(16 * 4 * 4, 30, ActivationFn::ReLU, 0.0)), // no dropout
